pipeline {
    ...
    environment {
        ...
        DOCKER_CREDENTIALS_ID = credentials('dockerhub_credentials')
    }

    stages {
        stage('Checkout Code') {
            steps {
                git 'https://github.com/devesh-singhal/Jan2025RestAssuredAPIFramework.git'
            }
        }
        ...
        stage('Push Docker Image to Docker Hub') {
            steps {
                withCredentials([usernamePassword(
                    ...
                    bat '''
                        echo "$DOCKER_PASS" | docker login -u "$DOCKER_USER" --password "$DOCKER_PASS"
